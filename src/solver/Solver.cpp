...
Solver::Solver(Map &map, const std::map<int, Agent> &agents, StatsManager &statsManager) : map(map), agents(agents), statsManager(statsManager) {
...
bool Solver::checkStateValidity(const State &state, const std::vector<std::vector<MapSquare>> &grid) {
    ...
    for (auto& it : state.getSearchSquares()) {
        ...
        if (known_pos.find(key) == known_pos.end()) {
            ...
            std::cerr << "pos error : "  << key << std::endl;
            return false;
        }
    }
