...
public:
    ...
    std::map<int, std::vector<std::shared_ptr<SearchSquare>>> lowLevelSolver(ConstraintNode &constraint_node, int agent_id = 0);
    ...
    static std::vector<std::shared_ptr<SearchSquare>>
    extractPathFromFinalSquare(const int &agent_id, const std::shared_ptr<SearchSquare> &final_seach_square);
};
